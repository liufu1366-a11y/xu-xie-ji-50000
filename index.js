...
// V56：一次 AI 生成/保存只占用一个撤回步骤。
// 生成预览、HUD、自动润色和保存后处理产生的中间 DOM 不进入撤回栈。
function commitGenerationHistory(baseContent) {
  if (!editorDom || isEditorDestroyed) return;
  const finalContent = editorDom.find("#xuxieji_editor_textarea").html();
  const finalPlainText = getEditorPlainText();
  const base = String(baseContent ?? "");

  // 找到生成前的真实原文；若历史栈中不存在，则补入一次。
  let baseIndex = -1;
  for (let i = historyStack.length - 1; i >= 0; i--) {
    if (historyStack[i]?.content === base) { baseIndex = i; break; }
  }
  if (baseIndex >= 0) {
    historyStack = historyStack.slice(0, baseIndex + 1);
    historyIndex = baseIndex;
  } else {
    historyStack = historyStack.slice(0, Math.max(0, historyIndex + 1));
    historyStack.push({ content: base, plainText: $("<div>").html(base).text() });
    historyIndex = historyStack.length - 1;
  }

  // V57：最终状态直接携带“本次 AI 生成前”的强制撤回目标。
  // 即使自动分章/自动润色曾产生“完整第2章”等中间正文，撤回也绝不经过它们。
  const basePlainText = $("<div>").html(base).text();
  if (historyStack[historyStack.length - 1]?.content !== finalContent) {
    historyStack.push({
      content: finalContent,
      plainText: finalPlainText,
      generationUndoTarget: { content: base, plainText: basePlainText }
    });
    historyIndex = historyStack.length - 1;
  } else {
    historyStack[historyStack.length - 1].generationUndoTarget = { content: base, plainText: basePlainText };
  }

  const maxSteps = extension_settings[extensionName].maxHistorySteps || defaultSettings.maxHistorySteps;
  while (historyStack.length > maxSteps + 1) {
    historyStack.shift();
...
function undoAction() {
  ...
  const forcedTarget = currentState?.generationUndoTarget;

  if (forcedTarget && typeof forcedTarget.content === "string") {
    // V57：AI生成事务撤回。直接回到生成前原文，不经过自动分章/润色/HUD中间态。
    let targetIndex = -1;
    for (let i = historyIndex - 1; i >= 0; i--) {
      if (historyStack[i]?.content === forcedTarget.content ||
          historyStack[i]?.plainText === forcedTarget.plainText) {
        targetIndex = i;
        break;
      }
    }
    if (targetIndex < 0) {
      historyStack.splice(historyIndex, 0, {
        content: forcedTarget.content,
        plainText: forcedTarget.plainText
      });
      targetIndex = historyIndex;
      historyIndex += 1; // 最终状态因插入前一位而右移
    }
    historyIndex = targetIndex;
    editorDom.find("#xuxieji_editor_textarea").html(forcedTarget.content);
  } else {
    historyIndex--;
...
function saveMechaDynamicState(s){ try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V48] 状态保存失败",e);} }
...
function buildMechaStateSpec(s){const rows=[];if(s.scan)rows.push({label:`扫描分析情况 · ${formatMechaUiTimestamp()}`,value:s.scan});rows.push({label:"时间",value:s.storyTime},{label:"电量",value:`${Number(s.battery).toFixed(1)}%`},{label:"网络",value:s.network},{label:"CPU",value:`${Math.round(s.cpu)}%`},{label:"GPU",value:`${Math.round(s.gpu)}%`},{label:"内存",value:`${Math.round(s.memory)}%`},{label:"位置",value:s.location},{label:"冷却液",value:`${Number(s.coolant).toFixed(1)}%`},{label:"温度",value:`${Number(s.temperature).toFixed(1)}°C`},{label:"损伤",value:s.damage},{label:"运行时长",value:formatMechaRuntime(s.runtimeSeconds)});return{show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V54",rows,alert:s.alert||"",footer:s.footer||""};}
async function analyzeMechaDynamicSegment(segment,previous){
 const p={...getDefaultMechaDynamicState(),...(previous||{})};
 const t=String(segment||"");
 const scanHit=/(扫描|侦测|识别|锁定|分析|电子眼|传感器|雷达|热成像|检索)/.test(t);
 const combat=/(战斗|攻击|爆炸|中弹|受击|冲击|枪|炮|追击|急刹|高速)/.test(t);
 const damageHit=/(损伤|断裂|缺失|破损|故障|受损|击穿|撕裂)/.test(t);
 const charge=/(充电|补能|能源补给)/.test(t);
 const rest=/(休息|待机|静止|睡眠)/.test(t);
 const elapsed=Math.max(5,Math.min(1800,Math.round(t.length/3)));
 const next={...p};
 // 正文明示数值优先：避免“87.5% -> 充电后 37%”这类状态跳变。
 const pctMatches=[...t.matchAll(/(?:电量|电池|核心电量|能源|供能)[^。！？%]{0,18}?(\d{1,3}(?:\.\d+)?)\s*%/g)];
 const barePct=[...t.matchAll(/(?:^|[。！？\n\s])\s*(\d{1,3}(?:\.\d+)?)\s*%/g)];
 const explicitBattery=(pctMatches.length?pctMatches[pctMatches.length-1]:barePct.length?barePct[barePct.length-1]:null);
 const timeMatch=t.match(/(\d{4}年\d{1,2}月\d{1,2}日(?:\s*\d{1,2}:\d{2})?|(?:清晨|早晨|上午|中午|下午|傍晚|晚上|夜晚|深夜)[^。！？\n]{0,12})/);
 if(timeMatch) next.storyTime=timeMatch[1].replace(/\s+/g,"");
 const locMatch=t.match(/(?:抵达|到达|进入|来到|位于|藏身于|躲在|停在|降落在)\s*([^，。！？\n]{2,28})/);
 if(locMatch) next.location=locMatch[1].trim();
 if(/断网|离线|网络中断|失去连接/.test(t)) next.network="OFFLINE";
 else if(/联网|网络恢复|重新连接|连接网络|信号恢复/.test(t)) next.network="ONLINE";
 next.runtimeSeconds=(Number(p.runtimeSeconds)||0)+elapsed;
 next.cpu=mechaClamp((Number(p.cpu)||12)+(scanHit?8:0)+(combat?12:0)-(rest?3:0),0,100,12);
 next.gpu=mechaClamp((Number(p.gpu)||8)+(scanHit?12:0)+(combat?8:0)-(rest?2:0),0,100,8);
 next.memory=mechaClamp((Number(p.memory)||18)+(scanHit?4:0),0,100,18);
 next.temperature=mechaClamp((Number(p.temperature)||36.5)+(combat?3.5:0)+(scanHit?1.2:0)-(rest?0.8:0),-100,300,36.5);
 next.coolant=mechaClamp((Number(p.coolant)||100)-(combat?0.8:0)-(scanHit?0.2:0),0,100,100);
 const prevBattery=mechaClamp(p.battery,0,100,100);
 const explicitEnergyFailure=/(核心.*(老化|损坏|故障|失效)|供能.*(故障|中断)|电池.*(损坏|故障)|更换.*核心.*(仅|只有|剩余).*\d+(?:\.\d+)?%)/.test(t);
 let batteryDelta=-(combat?0.7:0)-(scanHit?0.2:0)+(charge?5:0);
 next.battery=explicitBattery ? mechaClamp(Number(explicitBattery[1]),0,100,prevBattery) : mechaClamp(prevBattery+batteryDelta,0,100,prevBattery);
 // 连续状态保护：正文明确“充电/补能”时，除非同时明确能源故障或低电量替换核心，否则电量绝不能倒退。
 if(charge&&!explicitEnergyFailure&&next.battery<prevBattery) next.battery=prevBattery;
 if(damageHit) next.damage="检测到剧情中的机体损伤事件";
 next.scan=scanHit ? t.slice(Math.max(0,t.length-180)).replace(/\s+/g," ").slice(0,180) : "";
 next.alert=(combat&&damageHit)?"检测到战斗/损伤事件":"";
 next.footer=scanHit?"电子眼扫描数据已由本轮正文同步更新":"本轮机体状态已由正文同步更新";
 return next;
}
...
function createMechaPreviewCard(spec) {
  ...
  if(spec?.kind === "mini"){
    card.className = "xuxieji-mecha-ui xj-mini-hud" + (spec?.alert ? " mh-warn" : "");
    card.innerHTML = rows.map(row => `<span style="display:block"><span class="mh-k">${iconFor(row?.label)} ${escapeMechaInline(cleanLabel(row?.label))}</span><span>${escapeMechaInline(row?.value||"—")}</span></span>`).join("");
    return card;
  }
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V54")}</i>
    </span>
...
// V50：按“剧情状态变化事件”插入小 HUD；结尾再生成一次完整 HUD。
// 不依赖模型额外输出标签，普通生成同样生效。
function findMechaEventPoints(text){
  const s=String(text||""),pts=[];
  const rx=/(电子眼|机械眼|视觉传感器|光学传感器|视觉模块|瞳孔HUD|视野HUD|AR视野|战术视野|电子脑|机械意识|意识核心|思维模块|逻辑核心|运算核心|内部线程|后台进程|内部诊断|自检程序|扫描|侦测|识别|锁定|测距|热成像|红外|光谱|生命体征|目标分析|数据分析|充电|补能|能源补给|电量|电池|核心电量|供能|断电|低电量|损伤|受损|破损|断裂|故障|击穿|修复|维修|更换部件|网络|联网|离线|断网|连接|温度|过热|冷却|CPU|GPU|内存|位置|抵达|到达|离开|进入|撤离|小时|分钟|秒|天后|翌日|第二天|清晨|早晨|中午|下午|傍晚|夜晚|深夜)/g;
  let m;
  while((m=rx.exec(s))){
    let idx=m.index,b=s.slice(0,idx);
    let bd=Math.max(b.lastIndexOf("\n\n"),b.lastIndexOf("。"),b.lastIndexOf("！"),b.lastIndexOf("？"));
    if(bd>=0)idx=bd+1;
    // 太密集的同一段事件合并，避免一段话刷多个 HUD。
    if(!pts.length || idx-pts[pts.length-1] >= 120) pts.push(idx);
  }
  return [...new Set(pts.filter(n=>n>=0&&n<s.length))].sort((a,b)=>a-b).slice(0,10);
}

function buildMechaMiniSpec(state, segment){
  const t=String(segment||"");
  const rows=[];
  const add=(label,value)=>rows.push({label,value});
  if(/充电|补能|能源|电量|电池|供能|断电|低电量/.test(t)) add("电量",`${Number(state.battery).toFixed(1)}%`);
  if(/扫描|侦测|识别|锁定|分析|电子眼|传感器|雷达|热成像|检索/.test(t)) add("扫描",state.scan||"扫描完成");
  if(/损伤|受损|破损|断裂|故障|击穿|修复|维修/.test(t)) add("损伤",state.damage);
  if(/网络|联网|离线|断网|连接/.test(t)) add("网络",state.network);
  if(/温度|过热|冷却/.test(t)) { add("温度",`${Number(state.temperature).toFixed(1)}°C`); add("冷却液",`${Number(state.coolant).toFixed(1)}%`); }
  if(/CPU|GPU|内存|运算|电子脑|逻辑|意识|思维/.test(t)) { add("CPU",`${Math.round(state.cpu)}%`); add("GPU",`${Math.round(state.gpu)}%`); }
  if(/位置|抵达|到达|离开|进入|撤离/.test(t)) add("位置",state.location);
  if(/小时|分钟|秒|天后|翌日|第二天|清晨|早晨|中午|下午|傍晚|夜晚|深夜/.test(t)) { add("时间",state.storyTime); add("运行时长",formatMechaRuntime(state.runtimeSeconds)); }
  if(!rows.length) add("状态",state.footer||"剧情状态已更新");
  return {show:true,kind:"mini",title:"S-R-1090",badge:"LIVE",rows:rows.slice(0,3),alert:state.alert||"",footer:""};
}
...
function scheduleMechaPreviewUi(storyText) {
  ...
  // V54：机娘上下文 + 自身系统事件双条件触发；禁止裸关键词触发。
  // HUD 可替换明确的系统输出，也可替换明确描述机娘自身状态变化的叙事句。
  // 普通人物/环境叙事中的温度、位置、时间、损伤、意识等仍绝不触发 HUD。
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      if(token!==mechaPreviewUiToken)return;
      const replacements=await generateChatUIReplacements(input);
      if(token!==mechaPreviewUiToken)return;
      ...
      // 必须能在原文逐字定位，才能生成 HUD；定位不到就保持原文，绝不猜。
      const located=[];
      for(const item of (Array.isArray(replacements)?replacements:[])){
        const src=String(item?.source_text||"");
        if(src.trim().length<3)continue;
        let from=0,idx=-1;
        while((idx=input.indexOf(src,from))>=0){
          if(!located.some(x=>x.index===idx&&x.source===src)){
            located.push({index:idx,source:src,spec:{
              show:true,
              kind:item.kind||"system",
              title:item.title||"系统信息",
              subtitle:item.subtitle||"",
              badge:item.badge||"LIVE",
              rows:Array.isArray(item.rows)?item.rows.slice(0,10):[],
              alert:item.alert||"",
              footer:item.footer||""
            }});
            break;
      ...
      for(const x of located){
        if(x.index<cursor)continue;
        node.appendChild(node.ownerDocument.createTextNode(input.slice(cursor,x.index)));
        node.appendChild(createMechaPreviewCard(x.spec));
        cursor=x.index+x.source.length;
        rendered++;
      }
      node.appendChild(node.ownerDocument.createTextNode(input.slice(cursor)));
      previewSpan.addClass("xuxieji-ai-continuation-mark");

      // 没有明确系统输出时就是纯正文：不追加结尾状态 HUD，不生成默认 CPU/GPU/温度。
      mechaPendingState=null;
      console.log(`[续写鸡·MECHA V54] 上下文 HUD：${rendered} 个明确系统块`);
    }catch(error){
      console.error("[续写鸡·MECHA V54] HUD失败",error);
      if(!editorDom||isEditorDestroyed||!hasLivePreviewSpan())return;
      const previewSpan=editorDom.find("#preview_content_span");
      previewSpan.html(escapeHtml(input));
      previewSpan.addClass("xuxieji-ai-continuation-mark");
    }
  },300);
}
...
async function savePreviewContent() {
  ...
  // V56：保存事务开始。originalEditorContent 是点击 AI 继续前的真实原文。
  const generationBaseContent = String(originalEditorContent || (escapeHtml(beforeForSave) + escapeHtml(afterForSave)));
  const previousHistoryProcessing = isHistoryProcessing;
  isHistoryProcessing = true;
  ...
  if(mechaPendingState && mechaPendingState.branchIndex===currentSelectedBranchIndex && mechaPendingState.state){
    saveMechaDynamicState(mechaPendingState.state);
    try{
      const T=window.top||window,s=mechaPendingState.state;
      if(typeof T.MechaOSStoryUpdate==="function") T.MechaOSStoryUpdate({power:s.battery,cpu:s.cpu,gpu:s.gpu,memory:s.memory,temperature:s.temperature,coolant:s.coolant,location:s.location,network:s.network,storyTime:s.storyTime,uptimeSeconds:s.runtimeSeconds,damage:s.damage?[String(s.damage)]:[],vision:s.scan||"",activity:s.footer||"",log:s.scan?("扫描："+s.scan):(s.alert||"续写鸡状态同步")});
    }catch(e){console.warn("[续写鸡·MECHA V50] 主HUD即时同步失败",e);}
  }
  mechaPendingState=null;
...
editorDom.find("#preview_operation_container").hide().empty();
  ...
  // V56：整个生成/润色/HUD/后处理结束后，只把“最终正文”提交为一个历史步骤。
  isHistoryProcessing = previousHistoryProcessing;
  commitGenerationHistory(generationBaseContent);
...
${buildLengthInstruction(targetWordCount)}`;
  ...
  const raw = await generateRawWithOptionalStreaming(finalOptions, (full) => {
    ...
    try {
      if (!editorDom || isEditorDestroyed) return;
      const before = currentGenerationMode === "replace-selection" ? replacementBeforeText : cursorBeforeText;
      const after = currentGenerationMode === "replace-selection" ? replacementAfterText : cursorAfterText;
      const liveText = cleanTextFormat(String(full || ""));
      editorDom.find("#xuxieji_editor_textarea").html(
        `${escapeHtml(before)}<span id="preview_content_span" class="xuxieji-ai-continuation-mark fade-in" contenteditable="false">${escapeHtml(liveText)}</span>${escapeHtml(after)}`
      );
      if (liveText.length >= 80) showMechaPreviewLoading();
      const el = editorDom.find("#xuxieji_editor_textarea")[0];
      if (el) el.scrollTop = el.scrollHeight;
      updateWordCount();
    } catch (e) {
      console.warn("[续写鸡·MECHA V48] 同步正文/HUD预览失败", e);
    }
  });
...
async function generateThreeBranchesOnce(prompt, generateParams, originalBeforeText, targetWordCount) {
  ...
  const context = getContext();
  const settings = extension_settings[extensionName];
  if (getBranchCount() === 1 || settings.streamingSingleBranchEnabled) {
...
【A. 找出应该显示 HUD 的“机娘自身系统事件”】
HUD 不要求原文必须写成【系统：……】格式。只要当前事件明确发生在机娘/机器人自身，并且正文正在描述她的内部机械系统、电子眼、电子脑、能源、冷却、机体诊断等真实状态变化，就应该生成 HUD。

应该触发，例如：
- “电量达到100%，系统触发最终充能反馈机制”
- “电量：100%（满载）。启动核心冷却排放”
- “电子眼中的蓝光增强/扫描目标/识别目标”
- 电子脑重启、逻辑模块异常、内部诊断、自检、系统警告
- 明确的机体损伤、修复、断电、充电、冷却、网络连接变化
- 本地数据库/离线缓存检索、导航、任务系统更新

不要触发：
- 普通人物的时间、地点、温度、受伤、情绪、意识、动作
- 环境温度、普通地点变化、普通战斗描写
- 仅仅因为正文出现“CPU/GPU/时间/位置/损伤/温度”等关键词
- 与机娘自身系统无关的内容

核心判定必须同时满足：
1. 能明确判断事件主体是机娘/机器人；
2. 内容确实属于该机娘自身的机械/电子/能源/诊断状态或系统动作。
两条缺一不可。宁可少显示，也绝不能随机显示。

source_text 必须逐字复制原文中最能代表该系统事件的一小段。可以是系统格式文字，也可以是明确描述系统状态变化的叙事句。不要吞掉大段普通剧情。
同一连续事件（例如 70%→80%→95%→99%→100% 的一次充电过程）优先合并为一个 HUD；相隔较远、性质不同的系统事件分别生成 HUD。
...
function applyStoryState(
    ...
    if (typeof data.uptimeSeconds === 'number' && Number.isFinite(data.uptimeSeconds)) {
        mechaState.uptimeSeconds = Math.max(0, Math.floor(data.uptimeSeconds));
    }
...
(function(){
 ...
 setInterval(function(){try{
   ...
   T.MechaOSStoryUpdate({power:s.battery,cpu:s.cpu,gpu:s.gpu,memory:s.memory,temperature:s.temperature,
   coolant:s.coolant,location:s.location,network:s.network,storyTime:s.storyTime,
   uptimeSeconds:s.runtimeSeconds,
...
/* ===== V49 INLINE MINI HUD ===== */
(function(){
 const W=window.top||window,D=W.document;
 if(W.__XUXIEJI_V49_MINIHUD__)return; W.__XUXIEJI_V49_MINIHUD__=1;
 const st=D.createElement("style");
 st.textContent=`
 .xj-mini-hud{display:block;margin:9px 2px;padding:7px 10px;border:1px solid #237080;border-left:3px solid #35e9ff;border-radius:5px;background:#071f27;color:#bdf7ff;font:14px/1.55 monospace;box-shadow:0 0 10px #1bdff522}
 .xj-mini-hud .mh-k{color:#52ecff;font-weight:700;margin-right:7px}.xj-mini-hud.mh-warn{border-left-color:#ffd24a}.xj-mini-hud.mh-brain{border-left-color:#b69cff}.xj-mini-hud.mh-damage{border-left-color:#ff8b72}
 `;
 (D.head||D.documentElement).appendChild(st);

 function classify(s){
   if(/警告|警报|危险|异常|错误率|病毒|篡改|入侵/.test(s))return["⚠ 系统警告","mh-warn"];
   if(/损伤|受损|断裂|破损|故障|击穿|缺失/.test(s))return["🛠 损伤检测","mh-damage"];
   if(/主脑|电子脑|逻辑|计算|意识|思维|内心|情绪模拟/.test(s))return["🧠 电子脑","mh-brain"];
   if(/扫描|检测|侦测|识别|锁定|分析|电子眼|传感器|雷达|热成像/.test(s))return["🔎 电子眼",""];
   return null;
 }
 function convert(root){
   ...
   nodes.forEach(n=>{
     if(n.parentElement?.closest(".xj-mini-hud,script,style"))return;
     const v=n.nodeValue||"";
     const hasBracket=/[【\[]/.test(v);
     const hasTagged=/(?:^|\n)\s*#(?:正文|补充|警告|扫描|检测|状态)[^\n]{3,220}/.test(v);
     if(!hasBracket&&!hasTagged)return;
     // 兼容模型普通生成常见的 #正文 / #补充 状态行，把它们也转成文中小 HUD。
     if(hasTagged&&!hasBracket){
       const lines=v.split(/(\n)/); const frag=D.createDocumentFragment(); let changed=false;
       for(const part of lines){
         if(part==='\n'){frag.append(D.createTextNode(part));continue;}
         const mm=part.match(/^\s*#(正文|补充|警告|扫描|检测|状态)\s*(.+)$/);
         if(!mm){frag.append(D.createTextNode(part));continue;}
         const body=mm[2].trim(), c=classify(body)||[mm[1]==='警告'?'⚠ 系统警告':'◈ 状态信息',mm[1]==='警告'?'mh-warn':''];
         const box=D.createElement('span');box.className='xj-mini-hud '+c[1];
         box.innerHTML='<span class="mh-k">'+c[0]+'</span><span>'+body.replace(/[&<>]/g,x=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[x]))+'</span>';
         frag.append(box);changed=true;
       }
       if(changed){n.replaceWith(frag);return;}
     }
     const re=/[【\[]([^【】\[\]\n]{4,220})[】\]]/g;
     let m,last=0,frag=null;
     while((m=re.exec(v))){
       const c=classify(m[1]); if(!c)continue;
       if(!frag)frag=D.createDocumentFragment();
       frag.append(D.createTextNode(v.slice(last,m.index)));
       const box=D.createElement("span"); box.className="xj-mini-hud "+c[1];
       box.innerHTML='<span class="mh-k">'+c[0]+'</span><span>'+m[1].replace(/[&<>]/g,x=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[x]))+'</span>';
       frag.append(box); last=m.index+m[0].length;
     }
     if(frag){frag.append(D.createTextNode(v.slice(last)));n.replaceWith(frag);}
   });
 }
 function scan(){
   ...
   scan._busy=true;
   try{
     const roots=[D.querySelector("#xuxieji_editor_textarea"),D.querySelector(".xuxieji-editor-content"),D.querySelector("#preview_content_span")];
     scan._cache=scan._cache||new WeakMap();
     for(const root of roots){
       if(!root||!root.isConnected)continue;
       const text=(root.innerText??root.textContent??"").replace(/\s+/g," ").trim();
       const sig=text.length+"|"+text.slice(0,180)+"|"+text.slice(-180);
       if(scan._cache.get(root)===sig)continue;
       scan._cache.set(root,sig);
       convert(root);
       const after=(root.innerText??root.textContent??"").replace(/\s+/g," ").trim();
       scan._cache.set(root,after.length+"|"+after.slice(0,180)+"|"+after.slice(-180));
     }
   }finally{scan._busy=false;}
 }
 let scanTimer=0;
 function scheduleScan(){clearTimeout(scanTimer);scanTimer=setTimeout(scan,180);}
 const miniHudObserver=new MutationObserver(records=>{
   for(const r of records){
     const el=r.target?.nodeType===1?r.target:r.target?.parentElement;
     if(el?.closest?.(".xj-mini-hud"))continue;
     if(el?.closest?.("#xuxieji_editor_textarea,.xuxieji-editor-content,#preview_content_span")){scheduleScan();return;}
     for(const n of (r.addedNodes||[])){
       const ae=n.nodeType===1?n:n.parentElement;
       if(ae?.matches?.("#xuxieji_editor_textarea,.xuxieji-editor-content,#preview_content_span")||ae?.querySelector?.("#xuxieji_editor_textarea,.xuxieji-editor-content,#preview_content_span")){scheduleScan();return;}
     }
   }
 });
 miniHudObserver.observe(D.body,{subtree:true,childList:true,characterData:true});
 scheduleScan();
})();
