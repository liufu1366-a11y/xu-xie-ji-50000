...
function loadMechaDynamicState(){
  let local=getDefaultMechaDynamicState();
  try{const r=localStorage.getItem(getMechaDynamicStateStorageKey());if(r)local={...local,...JSON.parse(r)};}catch(e){}
  try{
    const W=window.top||window;
    if(typeof W.MechaOSGetState==="function"){
      const h=W.MechaOSGetState()||{};
      local={...local,
        battery:Number.isFinite(Number(h.battery))?Number(h.battery):local.battery,
        cpu:Number.isFinite(Number(h.cpu))?Number(h.cpu):local.cpu,
        gpu:Number.isFinite(Number(h.gpu))?Number(h.gpu):local.gpu,
        memory:Number.isFinite(Number(h.memory))?Number(h.memory):local.memory,
        temperature:Number.isFinite(Number(h.temperature))?Number(h.temperature):local.temperature,
        coolant:Number.isFinite(Number(h.coolant))?Number(h.coolant):local.coolant,
        location:h.location||local.location,network:h.network||local.network,
        storyTime:h.storyTime||local.storyTime,
        damage:Array.isArray(h.damage)&&h.damage.length?h.damage.join("；"):local.damage
      };
    }
  }catch(e){}
  return local;
}:getDefaultMechaDynamicState();}catch(e){return getDefaultMechaDynamicState();} }
function saveMechaDynamicState(s){
  const state={...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()};
  try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify(state));}catch(e){console.warn("[续写鸡·MECHA V39] 状态保存失败",e);}
  try{
    const W=window.top||window;
    if(typeof W.MechaOSStoryUpdate==="function"){
      W.MechaOSStoryUpdate({
        battery:state.battery,cpu:state.cpu,gpu:state.gpu,memory:state.memory,
        temperature:state.temperature,coolant:state.coolant,location:state.location,
        network:state.network,storyTime:state.storyTime,
        damage:state.damage?[String(state.damage)]:[],
        vision:state.scan||"",activity:state.footer||"",
        log:state.scan?`扫描：${state.scan}`:"机体状态同步"
      });
    }
  }catch(e){console.warn("[续写鸡·MECHA V39] 主HUD同步失败",e);}
}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V39] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V39")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V39] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V39",
    ...
    }catch(error){console.error("[续写鸡·MECHA V39] HUD失败",error);}
  },450);
}
...
/* ===== V39 EMBEDDED MECHA OS ===== */
async function bootEmbeddedMechaOSV39(){
﻿(async function () {
'use strict';
...
function moV30ApplySpecState(spec, storyText){
    ...
    // V33：运行时长也只按剧情经过时间推进，并随本轮结尾快照冻结。
    // 不再读取现实时间；每轮 elapsedMinutes 不同，运行时长就相应累加。
    if(typeof u.uptimeSeconds==='number'){
        patch.uptimeSeconds=Math.max(0,Math.round(u.uptimeSeconds));
    }else if(typeof u.elapsedMinutes==='number'){
        patch.uptimeSeconds=Math.max(0,Math.round(Number(mechaState.uptimeSeconds||0)+Math.max(0,Number(u.elapsedMinutes)||0)*60));
    }else{
        patch.uptimeSeconds=Math.max(0,Math.round(Number(mechaState.uptimeSeconds||0)));
    }
...
updateClock = function(){
    const dateEl=root && root.querySelector ? root.querySelector('#mo-date') : null;
    const timeEl=root && root.querySelector ? root.querySelector('#mo-time') : null;
    const frozen=String(mechaState.storyTime||'--:--:--');
    if(timeEl) timeEl.textContent=frozen;
    if(dateEl) dateEl.textContent=String(mechaState.storyDate||'STORY');
};

tickRuntimeState = function(){ return; };
...
(function(){
  ...
  function stateRows(forced){
    const s=forced||frozen()||{};
    const arr=[];
    const add=(label,value)=>{if(value!==undefined&&value!==null&&String(value)!=='')arr.push({label,value:String(value)});};
    add('时间',s.storyTime||'--:--:--');
    add('电量',typeof s.power==='number'?`${Number(s.power).toFixed(1).replace(/\.0$/,'')}%`:s.power);
    add('网络',s.network||'OFFLINE');
    add('CPU',typeof s.cpu==='number'?`${s.cpu}%`:s.cpu);
    if(typeof s.gpu==='number'||s.gpu)add('GPU',typeof s.gpu==='number'?`${s.gpu}%`:s.gpu);
    add('内存',typeof s.memory==='number'?`${s.memory}%`:s.memory);
    add('位置',s.location);
    add('冷却液',typeof s.coolant==='number'?`${Number(s.coolant).toFixed(1).replace(/\.0$/,'')}%`:s.coolant);
    add('温度',typeof s.temperature==='number'?`${s.temperature}°C`:s.temperature);
    if(Array.isArray(s.damage)&&s.damage.length)add('损伤',s.damage.join(' / '));
    return arr;
  }
  function compactRender(textBox,spec){
    ...
    card.dataset.kind=String(spec.kind||'system');
    const statusKeys=new Set(['时间','电量','网络','CPU','GPU','内存','位置','冷却液','温度','损伤']);
    ...
    // 剧情分析项优先，然后补当前冻结状态；避免重复。
    storyRows.forEach(push); stateRows(spec.__frozenState).forEach(push);
    const thought=spec.thought&&spec.thought.show!==false?spec.thought:null;
    const nav=spec.navigation&&spec.navigation.show===true?spec.navigation:null;
    const target=spec.targetAnalysis&&spec.targetAnalysis.show===true?spec.targetAnalysis:null;
    let scan='';
...
(function(){
  ...
  function v32StateRows(s){
    ...
    return [
      ...
      {label:'🧠 CPU',value:v32fmtPct(s.cpu)},
      {label:'🎛️ GPU',value:v32fmtPct(s.gpu)},
      {label:'💾 内存',value:v32fmtPct(s.memory)},
      {label:'📍 位置',value:s.location||'—'},
      {label:'💧 冷却液',value:v32fmtPct(s.coolant)},
      {label:'🌡️ 温度',value:typeof s.temperature==='number'?s.temperature+'°C':(s.temperature||'—')},
  ...
  W.MechaOSGenerateChatUI=async function(element,storyText){
    ...
    const statusWords=/(时间|电量|网络|CPU|GPU|内存|位置|冷却液|温度|损伤)/i;
