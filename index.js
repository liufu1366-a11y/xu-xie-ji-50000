...
async function analyzeMechaDynamicSegment(segment,previous){
 ...
 next.battery=mechaClamp(prevBattery+batteryDelta,0,100,prevBattery);
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V49")}</i>
    </span>
    ...
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V49",
