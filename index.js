...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V53")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  // V53：彻底取消“按关键词猜位置 + 自动编 CPU/GPU/温度等状态”的旧逻辑。
  // HUD 只允许替换正文里真实存在的、明确属于机娘系统/电子眼/电子脑的系统输出块。
  // 普通叙事里出现“温度、位置、时间、损伤、意识”等词，不再触发 HUD。
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      console.log(`[续写鸡·MECHA V53] 严格 HUD：${rendered} 个明确系统块`);
    }catch(error){
      console.error("[续写鸡·MECHA V53] HUD失败",error);
      if(!editorDom||isEditorDestroyed||!hasLivePreviewSpan())return;
      const previewSpan=editorDom.find("#preview_content_span");
