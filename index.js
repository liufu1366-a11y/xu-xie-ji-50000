...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V50")}</i>
    </span>
...
// V51：HUD 只在“机娘真实在场 + 当前段确实发生机娘感知/系统/机体事件”时出现。
// 禁止再按普通时间、地点、网络、温度等泛词乱插；也不再强制在全文结尾追加完整 HUD。
function isMechaContextSegment(text){
  const t=String(text||"");
  const explicit=/(机娘|机器少女|机械少女|仿生少女|仿生人|机器人|机械体|机体|电子眼|机械眼|视觉传感器|光学传感器|视觉模块|电子脑|机械意识|意识核心|思维模块|逻辑核心|运算核心|内部线程|后台进程|内部诊断|自检程序)/;
  const mechBody=/(钛合金|机械关节|液冷管|冷却液|伺服|执行器|传感器|机械躯体|合金骨架|仿生肌肉|核心电量|能源核心)/;
  return explicit.test(t)||mechBody.test(t);
}
function getMechaEventType(text){
  const t=String(text||"");
  if(!isMechaContextSegment(t)) return "";
  if(/(电子眼|机械眼|视觉传感器|光学传感器|视觉模块|扫描|侦测|识别|锁定|测距|热成像|红外|光谱|生命体征|目标分析|数据分析)/.test(t)) return "scan";
  if(/(电子脑|机械意识|意识核心|思维模块|逻辑核心|运算核心|内部线程|后台进程|内部诊断|自检程序|CPU|GPU|内存)/.test(t)) return "mind";
  if(/(充电|补能|能源补给|核心电量|电池|断电|低电量|损伤|受损|破损|断裂|故障|击穿|修复|维修|更换部件|过热|冷却液|液冷|伺服|执行器)/.test(t)) return "body";
  return "";
}
function findMechaEventPoints(text){
  const s=String(text||""),pts=[];
  // 先按自然段/句群分析上下文，只有机娘相关段落才允许触发。
  const boundary=/\n\s*\n|(?<=[。！？!?])\s*/g;
  let last=0,m;
  const chunks=[];
  while((m=boundary.exec(s))){
    const end=m.index+m[0].length;
    if(end>last) chunks.push({start:last,end,text:s.slice(last,end)});
    last=end;
  }
  if(last<s.length) chunks.push({start:last,end:s.length,text:s.slice(last)});
  for(const c of chunks){
    const type=getMechaEventType(c.text);
    if(!type) continue;
    // 同一小范围只保留一个事件 HUD。
    if(!pts.length || c.end-pts[pts.length-1].index>=120) pts.push({index:c.end,type,start:c.start,end:c.end});
  }
  return pts.slice(0,10);
}

function buildMechaMiniSpec(state, segment, eventType=""){
  const t=String(segment||"");
  const rows=[];
  const add=(label,value)=>rows.push({label,value});
  if(eventType==="scan") add("扫描",state.scan||"扫描完成");
  if(eventType==="mind") { add("CPU",`${Math.round(state.cpu)}%`); add("GPU",`${Math.round(state.gpu)}%`); }
  if(eventType==="body"){
    if(/充电|补能|能源|电量|电池|供能|断电|低电量/.test(t)) add("电量",`${Number(state.battery).toFixed(1)}%`);
    if(/损伤|受损|破损|断裂|故障|击穿|修复|维修|更换部件/.test(t)) add("损伤",state.damage);
    if(/温度|过热|冷却|液冷/.test(t)) { add("温度",`${Number(state.temperature).toFixed(1)}°C`); add("冷却液",`${Number(state.coolant).toFixed(1)}%`); }
  }
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      const events=findMechaEventPoints(input),rendered=[];
      let workingState=loadMechaDynamicState();
      if(!events.length){
      ...
      for(const ev of events){
        ...
        // 只分析事件附近上下文，避免把前后无关的人类剧情误塞进扫描 HUD。
        const segStart=Math.max(0,ev.start-220);
        const segment=input.slice(segStart,ev.end).trim();
        if(segment.length<12)continue;
        try{
          workingState=await analyzeMechaDynamicSegment(segment,workingState);
          const spec=buildMechaMiniSpec(workingState,segment,ev.type);
          if(spec&&spec.show===true) rendered.push({index:ev.end,spec});
        }catch(err){
          console.error("[续写鸡·MECHA V51] 动态扫描失败",err);
        }
      }
      if(token!==mechaPreviewUiToken)return;
      // V51：没有真实机娘事件就 0 HUD；有事件也只显示事件小 HUD，不再全文结尾强塞完整 HUD。
      mechaPendingState=rendered.length?{token,branchIndex:currentSelectedBranchIndex,state:{...workingState}}:null;
      if(!editorDom||isEditorDestroyed||!hasLivePreviewSpan())return;
    ...
    }catch(error){console.error("[续写鸡·MECHA V51] HUD失败",error);}
  },450);
}
...
(function(){
 ...
 function scan(){
   convert(D.querySelector("#xuxieji_editor_textarea"));
   convert(D.querySelector(".xuxieji-editor-content"));
   convert(D.querySelector("#preview_content_span"));
 }
 new MutationObserver(()=>scan()).observe(D.body,{subtree:true,childList:true,characterData:true});
