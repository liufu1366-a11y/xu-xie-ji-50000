...
function saveMechaDynamicState(s){
  ...
  try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify(state));}
  catch(e){console.warn("[续写鸡·MECHA V41] 状态保存失败",e);}
  try{
    ...
    if(typeof W.MechaOSStoryUpdate==="function"){
      W.MechaOSStoryUpdate({
        ...
        temperature:state.temperature,coolant:state.coolant,
        location:state.location,network:state.network,storyTime:state.storyTime,
        damage:state.damage?[String(state.damage)]:[],
        vision:state.scan||"",activity:state.footer||"",
        log:state.scan?("扫描："+state.scan):"续写鸡状态同步"
      });
    }
  }catch(e){console.warn("[续写鸡 V41] HUD状态同步失败",e);}
}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V41] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V41")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V41] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V41",
    ...
    }catch(error){console.error("[续写鸡·MECHA V41] HUD失败",error);}
  },450);
}
