...
function saveMechaDynamicState(s){ try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V36] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  const stateRows = frozenRows.length
    ? frozenRows
    : rows.filter(r => !/扫描|分析|目标/.test(String(r?.label||""))).slice(0,12);
...
function scheduleMechaPreviewUi(storyText) {
  const fallbackGenerator=getMechaPreviewGenerator();
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        let spec=null;
        try{workingState=await analyzeMechaDynamicSegment(segment,workingState);spec=buildMechaStateSpec(workingState);}
        catch(err){console.warn("[续写鸡·MECHA V36] 动态扫描失败",err);if(fallbackGenerator){try{spec=await fallbackGenerator(null,segment);}catch(e){}}}
    ...
    }catch(error){console.error("[续写鸡·MECHA V36] HUD失败",error);}
  },450);
}
