...
function createMechaPreviewCard(spec) {
  ...
  card.className = "xuxieji-mecha-ui";
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-ui-head">
      <span class="xuxieji-mecha-ui-headtext">
        <span class="xuxieji-mecha-ui-title">${escapeMechaInline(spec?.title || "SYSTEM ANALYSIS")}</span>
        <span class="xuxieji-mecha-ui-sub">${escapeMechaInline(spec?.subtitle || "S-R-1090 LOCAL SYSTEM")}</span>
      </span>
      <span class="xuxieji-mecha-ui-badge">${escapeMechaInline(spec?.badge || "READY")}</span>
    </span>

    <span class="xuxieji-mecha-ui-body">
      ${rows.map((row) => `
        <span class="xuxieji-mecha-ui-row">
          <span class="xuxieji-mecha-ui-label">${escapeMechaInline(row?.label || "DATA")}</span>
          <span class="xuxieji-mecha-ui-value">${escapeMechaInline(row?.value || "—")}</span>
        </span>
      `).join("")}
    </span>

    ${spec?.alert ? `
      <span class="xuxieji-mecha-ui-alert">⚠ ${escapeMechaInline(spec.alert)}</span>
    ` : ""}

    <span class="xuxieji-mecha-ui-footer">${escapeMechaInline(spec?.footer || "S-R-1090 · LOCAL PROCESSING")}</span>
  `;
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer = setTimeout(async () => {
    try {
      // 传 null，只让 MECHA OS 的文字模型生成结构化 spec；渲染交给续写鸡自己完成。
