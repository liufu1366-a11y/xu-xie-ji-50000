...
async function analyzeMechaDynamicSegment(segment,previous){
 ...
 next.battery=mechaClamp((Number(p.battery)||100)-(combat?0.7:0)-(scanHit?0.2:0)+(charge?5:0),0,100,100);
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V48")}</i>
    </span>
    ...
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V48",
...
(function(){
 ...
 if(W.__XUXIEJI_V48_MINIHUD__)return; W.__XUXIEJI_V48_MINIHUD__=1;
 ...
 function scan(){convert(D.querySelector("#xuxieji_editor_textarea"));convert(D.querySelector(".xuxieji-editor-content"));}
