...
// ============================================================
// MECHA OS × 续写鸡：把“系统文字”直接替换成 HUD
// 普通剧情不会插 UI；只有 MECHA 返回 exact source_text 才替换。
// ============================================================

let mechaPreviewUiTimer = null;
let mechaPreviewUiToken = 0;

function getMechaReplacementGenerator() {
  try {
    if (window.top && typeof window.top.MechaOSGenerateUIReplacements === "function") {
      return window.top.MechaOSGenerateUIReplacements;
    }
  } catch (e) {}

  try {
    if (typeof window.MechaOSGenerateUIReplacements === "function") {
      return window.MechaOSGenerateUIReplacements;
    }
  } catch (e) {}
...
function getMechaStateSnapshot() {
  try {
    if (window.top && typeof window.top.MechaOSGetState === "function") {
      return window.top.MechaOSGetState();
    }
  } catch (e) {}
...
function formatMechaUptime(totalSeconds) {
...
function createMechaPreviewCard(spec) {
  ...
  const state = getMechaStateSnapshot();
  const card = doc.createElement("span");

  card.className = "xuxieji-mecha-ui xuxieji-mecha-full-hud";
  card.setAttribute("contenteditable", "false");
  card.dataset.kind = String(spec?.kind || "system");

  const rows = Array.isArray(spec?.rows) ? spec.rows.slice(0, 8) : [];
  const systems = state.systems || {};

  card.innerHTML = `
    <span class="mecha-full-top">
      <span class="mecha-full-brand">
        <b>${escapeMechaInline(state.model || "S-R-1090")}</b>
        <small>VISUAL LINK : REALTIME MODE</small>
      </span>

      <span class="mecha-full-topitem">
        <small>SYSTEM STATUS</small>
        <b>系统状态：${spec?.alert ? "警告" : "正常"}</b>
      </span>

      <span class="mecha-full-topitem">
        <small>TIME / 时间</small>
        <b class="mecha-live-time">--:--:--</b>
      </span>

      <span class="mecha-full-topitem">
        <small>SIGNAL</small>
        <b class="mecha-live-network">${escapeMechaInline(state.network || "OFFLINE")}</b>
      </span>
    </span>

    <span class="mecha-full-main">

      <span class="mecha-full-left">
        <span class="mecha-panel-title">核心状态 <small>CORE STATUS</small></span>

        <span class="mecha-unit-diagram">
          <i class="mecha-head"></i>
          <i class="mecha-torso"></i>
          <i class="mecha-arm left"></i>
          <i class="mecha-arm right"></i>
          <i class="mecha-leg left"></i>
          <i class="mecha-leg right"></i>
          <i class="mecha-core-dot"></i>
        </span>

        <span class="mecha-live-statrow"><em>动力电量</em><b class="mecha-live-power">${Number(state.power || 0).toFixed(2)}%</b></span>
        <span class="mecha-live-statrow"><em>冷却液</em><b class="mecha-live-coolant">${Number(state.coolant || 0).toFixed(2)}%</b></span>
        <span class="mecha-live-statrow"><em>CPU负载</em><b class="mecha-live-cpu">${Math.round(state.cpu || 0)}%</b></span>
        <span class="mecha-live-statrow"><em>核心温度</em><b class="mecha-live-temp">${Number(state.temperature || 0).toFixed(1)}°C</b></span>
        <span class="mecha-live-statrow"><em>神经同步</em><b class="mecha-live-sync">${Math.round(state.sync || 0)}%</b></span>
      </span>

      <span class="mecha-full-center">

        <span class="mecha-center-head">
          <span>
            <small>${escapeMechaInline(spec?.subtitle || "S-R-1090 / SYSTEM ANALYSIS")}</small>
            <b>${escapeMechaInline(spec?.title || "系统分析")}</b>
          </span>
          <strong>${escapeMechaInline(spec?.badge || "ACTIVE")}</strong>
        </span>

        <span class="mecha-target-reticle">
          <i></i><i></i><i></i><i></i>
          <b>◎</b>
        </span>

        <span class="mecha-full-data">
          ${rows.length
            ? rows.map((row) => `
              <span class="mecha-full-datarow">
                <em>${escapeMechaInline(row?.label || "DATA")}</em>
                <b>${escapeMechaInline(row?.value || "—")}</b>
              </span>
            `).join("")
            : `
              <span class="mecha-full-datarow">
                <em>STATUS</em>
                <b>NO ADDITIONAL DATA</b>
              </span>
            `
          }
        </span>
        ...
        ${spec?.alert
          ? `<span class="mecha-full-alert">⚠ ${escapeMechaInline(spec.alert)}</span>`
          : ""
        }

        <span class="mecha-full-center-bottom">
          <span><small>CURRENT ACTIVITY</small><b class="mecha-live-activity">${escapeMechaInline(state.activity || "IDLE")}</b></span>
          <span><small>VISION MODE</small><b class="mecha-live-vision">${escapeMechaInline(state.vision || "NORMAL")}</b></span>
          <span><small>UPTIME</small><b class="mecha-live-uptime">${escapeMechaInline(formatMechaUptime(state.uptimeSeconds))}</b></span>
        </span>
      </span>

      <span class="mecha-full-right">
        <span class="mecha-panel-title">机体系统 <small>SYSTEMS</small></span>

        ${[
          ["动力系统", Number(systems.power ?? 100)],
          ["神经系统", Number(systems.neural ?? 99)],
          ["运动系统", Number(systems.movement ?? 100)],
          ["感知系统", Number(systems.perception ?? 100)],
          ["自修复系统", Number(systems.repair ?? 100)]
        ].map(([name, value]) => `
          <span class="mecha-full-system">
            <span><em>${escapeMechaInline(name)}</em><b>${Math.round(value)}%</b></span>
            <i><u style="width:${Math.max(0, Math.min(100, value))}%"></u></i>
          </span>
        `).join("")}

        <span class="mecha-full-info">
          <span><em>机体名称</em><b>${escapeMechaInline(state.name || "爱丽斯忒")}</b></span>
          <span><em>当前位置</em><b class="mecha-live-location">${escapeMechaInline(state.location || "UNKNOWN")}</b></span>
          <span><em>当前任务</em><b class="mecha-live-task">${escapeMechaInline(state.task || "UNKNOWN")}</b></span>
          <span><em>人格控制</em><b class="mecha-live-persona">${state.personaDirective ? "ACTIVE" : "STANDBY"}</b></span>
        </span>
      </span>
    </span>

    <span class="mecha-full-bottom">
      <span>◉ 视觉增强</span>
      <span>◎ 夜间视觉</span>
      <span>⌾ 目标锁定</span>
      <span>△ 环境扫描</span>
    ...
    <span class="mecha-full-footer">
      ${escapeMechaInline(spec?.footer || "S-R-1090 · LOCAL PROCESSING")}
    </span>
  `;

  // 续写鸡主题里存在 !important 文字颜色。
  // 这里直接给 HUD 写入 inline !important，保证不会再变成棕黑字。
  ...
  card.style.setProperty("background", "#020b12", "important");
  card.style.setProperty("color", "#eafcff", "important");
  card.style.setProperty("border-color", "#25cfee", "important");
  card.style.setProperty("text-decoration", "none", "important");
  card.style.setProperty("-webkit-text-fill-color", "#eafcff", "important");
  ...
  force("b, strong", {
    "color":"#65ecff",
    "-webkit-text-fill-color":"#65ecff"
  });

  force("em, small", {
    "color":"#9ab9c6",
    "-webkit-text-fill-color":"#9ab9c6"
  });

  force(".mecha-full-datarow b, .mecha-center-head b, .mecha-panel-title", {
    "color":"#f1fdff",
    "-webkit-text-fill-color":"#f1fdff"
  });

  force(".mecha-full-left, .mecha-full-right, .mecha-full-center", {
    "background-color":"#031019"
  });

  force(".mecha-full-alert", {
    "color":"#ff8a96",
    "-webkit-text-fill-color":"#ff8a96",
    "background":"#2b080d",
    "border-color":"#ff5164"
  });
...
function syncMechaFullHudCards() {
  if (!editorDom || isEditorDestroyed) return;
  ...
  editorDom.find(".xuxieji-mecha-full-hud").each(function() {
    const card = $(this);

    card.find(".mecha-live-time").text(time);
    card.find(".mecha-live-power").text(Number(state.power || 0).toFixed(2) + "%");
    card.find(".mecha-live-coolant").text(Number(state.coolant || 0).toFixed(2) + "%");
    card.find(".mecha-live-cpu").text(Math.round(state.cpu || 0) + "%");
    card.find(".mecha-live-temp").text(Number(state.temperature || 0).toFixed(1) + "°C");
    card.find(".mecha-live-sync").text(Math.round(state.sync || 0) + "%");
    card.find(".mecha-live-network").text(state.network || "OFFLINE");
    card.find(".mecha-live-activity").text(state.activity || "IDLE");
    card.find(".mecha-live-vision").text(state.vision || "NORMAL");
    card.find(".mecha-live-uptime").text(formatMechaUptime(state.uptimeSeconds));
    card.find(".mecha-live-location").text(state.location || "UNKNOWN");
    card.find(".mecha-live-task").text(state.task || "UNKNOWN");
    card.find(".mecha-live-persona").text(state.personaDirective ? "ACTIVE" : "STANDBY");
  });
}

setInterval(
  syncMechaFullHudCards,
  1000
);
...
function normalizeMechaSource(value) {
  return String(value || "")
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n");
}
...
function isMechaSystemLine(line) {
  ...
  if (!bracketLike) return false;

  return /(扫描|分析|检测|目标|锁定|识别|状态|系统|进度|评估|完整度|磨损|损伤|电量|剩余电量|冷却液|温度|CPU|内存|同步|信号|距离|威胁|种属|性别|年龄|身高|健康|改造|处理方案|导航|任务|数据库|查询|检索|网络|覆写|覆盖|电压|压制|模拟人格|人格|充电|能源|接口|功率|运行时间|故障|警告|异常|协议|轨迹|速度|ZOOM|TARGET|SCAN|STATUS|POWER|TEMP|MEMORY|SYSTEM|WARNING|ERROR)/i.test(text);
}

function parseSystemRowsFromBlock(source) {
  const rows = [];
...
function buildDeterministicSystemReplacements(text) {
  const raw = String(text || "");
  ...
  function flush() {
    ...
    const qualifying =
      block.filter(isMechaSystemLine);

    // 至少一条明显的系统输出；两条以上时必定做成HUD。
    if (
      source
      &&
      (
        ...
        (
          qualifying.length === 1
          &&
          /(扫描|分析|检测|目标|锁定|系统|警告|异常|数据库|检索|进度|充电|覆写|电压|压制)/i.test(source)
        )
      )
    ) {
      let kind = "system";
      ...
      if (/(扫描|识别|电子眼|目标)/i.test(source)) {
        kind = "scan";
        title = "电子视觉分析";
        badge = "SCAN";
      }

      if (/(损伤|完整度|磨损|故障|断裂)/i.test(source)) {
        kind = "damage";
        title = "机体损伤检测";
        badge = "DAMAGE";
      }

      if (/(数据库|检索|查询|网络)/i.test(source)) {
        kind = "database";
        title = "本地数据库检索";
        badge = "LOCAL";
      }

      if (/(充电|电量|冷却液|温度|功率|能源)/i.test(source)) {
        kind = "status";
        title = "机体能源状态";
        badge = "POWER";
      }

      if (/(覆写|压制|模拟人格|人格)/i.test(source)) {
        kind = "system";
        title = "人格控制状态";
        badge = "CONTROL";
      }
      ...
      replacements.push({
        ...
        kind,
        title,
        subtitle: "S-R-1090 / REALTIME SYSTEM",
        badge,
        rows: parseSystemRowsFromBlock(source),
        alert:
          /(警告|严重|异常|故障|极高|危险|OVERHEAT|ERROR|REJECTED)/i.test(source)
          ? "检测到异常状态"
          : "",
        footer: "S-R-1090 · LOCAL PROCESSING"
      });
    }
  ...
  for (const line of lines) {
    ...
    if (!trimmed) {
      if (active) {
        // 空行只允许在系统块内部出现一次，不立即结束
        block.push("");
      }
      continue;
    }

    if (isMechaSystemLine(trimmed)) {
      if (!active) {
        block = [];
        active = true;
      }
...
function locateExactReplacementSegments(text, replacements) {
  const raw = normalizeMechaSource(text);
  const found = [];

  function findFlexible(source, fromIndex) {
    const exact = raw.indexOf(source, fromIndex);
    if (exact >= 0) {
      return {
        start: exact,
        end: exact + source.length
      };
    }

    // 对换行/空格做容错，但仍然只在原文中定位连续区域。
    const parts =
      source
      .split(/\s+/)
      .filter(Boolean);
...
function renderReplacementPreview(originalText, replacements) {
  if (!editorDom || isEditorDestroyed) return false;
  ...
  const { raw, segments } =
    locateExactReplacementSegments(
      originalText,
      replacements
    );

  if (!segments.length) {
    previewSpan.html(escapeHtml(raw));
    previewSpan.addClass("xuxieji-ai-continuation-mark");
    return false;
  }
    ...
    if (item.start > cursor) {
      node.appendChild(
        node.ownerDocument.createTextNode(
          raw.slice(cursor, item.start)
        )
      );
    }

    // 这里不保留 source_text：原文字被 HUD 完全取代。
    node.appendChild(
      createMechaPreviewCard(
        item.spec
      )
    );
  ...
  // HUD 只替换显示层，底层继续保留原始系统文字，
  // 这样保存、切换分支、重新分析时不会丢失依据。
  currentBranchResults[currentSelectedBranchIndex] =
    cleanTextFormat(raw)
      .replace(/^[\s\n\r]+/g, "");

  lastGeneratedBranchResults =
    currentBranchResults.slice();

  return true;
}

function showMechaReplacementLoading() {
  if (!editorDom || isEditorDestroyed) return;

  const operationContainer =
    editorDom.find("#preview_operation_container");

  operationContainer
    .find(".xuxieji-mecha-replace-status")
    .remove();

  operationContainer.prepend(`
    <div class="xuxieji-mecha-replace-status">
...
function hideMechaReplacementLoading() {
  if (!editorDom || isEditorDestroyed) return;
...
function scheduleMechaPreviewUi(storyText) {
  const generator =
    getMechaReplacementGenerator();

  if (!generator) {
    console.warn(
      "[续写鸡·MECHA] 未发现 MechaOSGenerateUIReplacements；请确认新版 MECHA OS 配套脚本已启用。"
    );
    return;
  }

  if (mechaPreviewUiTimer) {
    clearTimeout(mechaPreviewUiTimer);
    mechaPreviewUiTimer = null;
  }
    ...
    setTimeout(
      async () => {
        ...
        try {
          ...
          const deterministic =
            buildDeterministicSystemReplacements(
              input
            );
...
function bindPreviewOperationEvents() {
  ...
  editorDom.find("#preview_edit_btn").on("click", (e) => {
    ...
    const previewSpan = editorDom.find("#preview_content_span");
    if (!isEditingPreview) {
      // MECHA：进入“修改”前恢复当前纯剧情文本，HUD 不作为可编辑文字。
      const currentPure = currentBranchResults[currentSelectedBranchIndex] || "";
      previewSpan.html(escapeHtml(currentPure));
      previewSpan.addClass("xuxieji-ai-continuation-mark");
...
function getMechaPersonaDirectiveForGeneration() {
  try {
    if (window.top && typeof window.top.MechaOSGetPersonaDirective === "function") {
      const value = String(window.top.MechaOSGetPersonaDirective() || "").trim();
      if (value) return value;
    }
  } catch (e) {}
  ...
  try {
    const topValue =
      window.top?.localStorage?.getItem(
        "MECHA_VIRTUAL_PERSONA_DIRECTIVE"
      );
    if (topValue) return String(topValue).trim();
  } catch (e) {}
...
function buildGenerateConfig() {
  ...
  const mechaPersonaDirective =
    getMechaPersonaDirectiveForGeneration();

  const mechaPersonaPrompt =
    mechaPersonaDirective
    ? `【S-R-1090 虚拟人格控制】
以下内容由用户从 MECHA OS 的“虚拟人格控制”面板写入。
当后续剧情涉及爱丽斯忒 / S-R-1090 的行为、对白、判断、反应与主动决策时，请自然遵循这些控制要求。
不要在正文里复述“控制指令”本身，也不要把它写成系统提示。
若它与已经发生且不可更改的事实冲突，以已发生事实为准。
