...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V52")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          spec=buildMechaMiniSpec(workingState,segment);
        }catch(err){
          console.error("[续写鸡·MECHA V48] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V52",
            rows:buildMechaMiniSpec(workingState,segment).rows,
      ...
      if(token!==mechaPreviewUiToken)return;
      // 事件小 HUD 负责过程；全文末尾追加一张完整 HUD，作为本轮最终冻结状态。
      if(rendered.length){
        const finalSpec=buildMechaStateSpec(workingState);
        finalSpec.kind="system";
        rendered.push({index:input.length,spec:finalSpec});
      }
    ...
    }catch(error){console.error("[续写鸡·MECHA V48] HUD失败",error);}
  },450);
}
