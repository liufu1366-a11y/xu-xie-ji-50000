  ...
  if (!doc.getElementById("xuxieji-mecha-v32-style")) {
    ...
    st.textContent=`
      ...
      .xuxieji-mecha-v32-head{display:flex!important;justify-content:space-between!important;align-items:center!important;
        ...
        font-size:9px!important}.xuxieji-mecha-v32-head b{color:#7eeeff!important}.xuxieji-mecha-v32-head i{
        font-style:normal!important;font-size:7px!important;padding:1px 4px!important;border:1px solid rgba(70,220,255,.3)!important;border-radius:9px!important}
      ...
      .xuxieji-mecha-v32-chip,.xuxieji-mecha-v32-scan>span{display:flex!important;justify-content:space-between!important;
        ...
        background:rgba(15,47,58,.55)!important;border:1px solid rgba(70,220,255,.09)!important;font-size:8px!important}
      ...
      .xuxieji-mecha-v32-note{display:block!important;margin-top:3px!important;padding:3px 4px!important;
        border-left:2px solid #43dfff!important;background:rgba(10,39,49,.55)!important;font-size:7.5px!important;color:#b9dce5!important}
      @media(max-width:390px){.xuxieji-mecha-v32-grid{grid-template-columns:1fr 1fr!important}
        .xuxieji-mecha-v32-chip{font-size:7.5px!important;padding:2px 3px!important}}
    `;
...
function scheduleMechaPreviewUi(storyText) {
  ...
  if (!generator) {
    console.warn("[续写鸡·MECHA] 未发现 MechaOSGenerateChatUI；请确认 MECHA OS 脚本已启用并已配置文本 API。");
    return;
  }
  ...
  mechaPreviewUiTimer = setTimeout(async () => {
    try {
      // V32：MECHA OS 在此调用中完成“本轮结尾状态结算+冻结”；续写鸡只渲染返回的冻结 spec。
      const spec = await generator(null, input);
      ...
      if (!spec || spec.show !== true) {
        // 模型判断本段无需 HUD：恢复纯文本，不插 UI。
        const pureText = getPreviewPureText(previewSpan);
        previewSpan.html(escapeHtml(pureText));
        previewSpan.addClass("xuxieji-ai-continuation-mark");
        return;
      }

      insertMechaUiIntoPreview(spec);
    } catch (error) {
      console.error("[续写鸡·MECHA] 生成聊天内 HUD 失败:", error);
      if (!editorDom || isEditorDestroyed || !hasLivePreviewSpan()) return;
      const previewSpan = editorDom.find("#preview_content_span");
      const pureText = getPreviewPureText(previewSpan);
      previewSpan.html(escapeHtml(pureText));
