...
function saveMechaDynamicState(s){ try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V42] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V42")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V42] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V42",
    ...
    }catch(error){console.error("[续写鸡·MECHA V42] HUD失败",error);}
  },450);
}
...
/* ===== V42 NATIVE MAIN HUD — same state source as continuation HUD ===== */
(function installXuxiejiNativeMainHudV42(){
  const W=window.top||window, D=W.document;
  if(!D || D.getElementById("xuxieji-v42-mainhud-btn")) return;

  const style=D.createElement("style");
  style.id="xuxieji-v42-mainhud-style";
  style.textContent=`
  #xuxieji-v42-mainhud-btn{position:fixed;right:18px;bottom:110px;z-index:2147483000;width:58px;height:58px;border-radius:50%;border:2px solid #27e8ff;background:#071b25;color:#8ff5ff;box-shadow:0 0 18px #13dff088;font-size:25px}
  #xuxieji-v42-mainhud{position:fixed;inset:8% 4%;z-index:2147483001;background:#081a22f5;border:1px solid #2adcf0;border-radius:18px;color:#bff7ff;padding:16px;overflow:auto;box-shadow:0 0 30px #10d9ee55;font-family:monospace}
  #xuxieji-v42-mainhud[hidden]{display:none!important}
  #xuxieji-v42-mainhud .v42head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #28616d;padding-bottom:10px;margin-bottom:12px}
  #xuxieji-v42-mainhud .v42grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
  #xuxieji-v42-mainhud .v42cell{border:1px solid #1e5c69;padding:9px;border-radius:7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  #xuxieji-v42-mainhud .v42wide{grid-column:1/-1;white-space:normal}
  #xuxieji-v42-mainhud button{background:#10303b;color:#c8fbff;border:1px solid #2adcf0;border-radius:8px;padding:7px 12px}`;
  D.head.appendChild(style);

  const btn=D.createElement("button");
  btn.id="xuxieji-v42-mainhud-btn"; btn.textContent="◎"; btn.title="MECHA OS V42";
  const panel=D.createElement("div");
  panel.id="xuxieji-v42-mainhud"; panel.hidden=true;
  panel.innerHTML='<div class="v42head"><b>MECHA OS · V42</b><button id="xuxieji-v42-close">关闭</button></div><div id="xuxieji-v42-body"></div>';
  D.body.appendChild(btn); D.body.appendChild(panel);

  function esc(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}
  function render(){
    let s={};
    try{s=loadMechaDynamicState()||{};}catch(e){}
    const rows=[
      ["时间",s.storyTime],["电量",(s.battery??"--")+"%"],
      ["网络",s.network],["CPU",(s.cpu??"--")+"%"],
      ["GPU",(s.gpu??"--")+"%"],["内存",(s.memory??"--")+"%"],
      ["位置",s.location],["冷却液",(s.coolant??"--")+"%"],
      ["温度",(s.temperature??"--")+"°C"],["损伤",s.damage],
      ["运行时长",typeof formatMechaRuntime==="function"?formatMechaRuntime(s.runtimeSeconds):s.runtimeSeconds]
    ];
    let html='<div class="v42grid">'+rows.map(x=>'<div class="v42cell"><b>'+esc(x[0])+'</b>　'+esc(x[1])+'</div>').join("");
    if(s.scan) html+='<div class="v42cell v42wide"><b>扫描分析</b><br>'+esc(s.scan)+'</div>';
    if(s.alert) html+='<div class="v42cell v42wide"><b>警报</b><br>'+esc(s.alert)+'</div>';
    html+='</div>';
    const body=D.getElementById("xuxieji-v42-body"); if(body) body.innerHTML=html;
  }
  btn.onclick=()=>{render();panel.hidden=!panel.hidden;};
  panel.querySelector("#xuxieji-v42-close").onclick=()=>{panel.hidden=true;};
  W.addEventListener("storage",render);
  setInterval(()=>{if(!panel.hidden)render();},1000);
})();
