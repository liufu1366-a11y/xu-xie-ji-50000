...
function saveMechaDynamicState(s){ try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V43] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V43")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V43] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V43",
    ...
    }catch(error){console.error("[续写鸡·MECHA V43] HUD失败",error);}
  },450);
}
...
/* ===== V43 COMPACT NATIVE HUD ===== */
(function(){
const W=window.top||window,D=W.document;if(!D||D.getElementById("v43orb"))return;
const st=D.createElement("style");st.textContent=`
#v43orb{position:fixed;right:18px;bottom:108px;z-index:2147483000;width:60px;height:60px;border-radius:50%;border:2px solid #25eaff;background:#061b24;color:#9ff8ff;box-shadow:0 0 20px #20eaff88;font-size:27px}
#v43hud{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:2147483001;width:min(86vw,560px);max-height:70vh;overflow:hidden;background:#071b23f7;border:1px solid #25dff0;border-radius:13px;color:#c4f9ff;box-shadow:0 0 26px #16e8ff55;font-family:monospace}
#v43hud[hidden]{display:none!important}.v43h{display:flex;align-items:center;padding:11px 12px;border-bottom:1px solid #235966}.v43h b{flex:1;color:#67edff}.v43h button,.v43tabs button{background:#0b2b35;color:#c8fbff;border:1px solid #286573;border-radius:6px;padding:6px 9px}
.v43tabs{display:flex;gap:5px;padding:7px 9px;border-bottom:1px solid #1b4b57;overflow:auto}.v43tabs button.on{border-color:#38edff;box-shadow:inset 0 -2px #38edff}.v43body{padding:10px;max-height:calc(70vh - 96px);overflow:auto}.v43g{display:grid;grid-template-columns:1fr 1fr;gap:6px}.v43c{border:1px solid #205765;background:#092630;padding:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.v43box{border-left:3px solid #32e9ff;background:#092630;padding:9px;margin:8px 0;line-height:1.5}`;
D.head.appendChild(st);
const o=D.createElement("button");o.id="v43orb";o.textContent="◎";
const h=D.createElement("div");h.id="v43hud";h.hidden=true;h.innerHTML='<div class="v43h"><b>S-R-1090 · MECHA OS V43</b><button>关闭</button></div><div class="v43tabs"><button data-p="s" class="on">机体</button><button data-p="v">电子眼</button><button data-p="b">电子脑</button><button data-p="l">日志</button></div><div class="v43body"></div>';
D.body.append(o,h);let p="s";const esc=x=>String(x??"").replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]));
function S(){try{return loadMechaDynamicState()||{}}catch(e){return{}}}function c(a,b){return '<div class="v43c"><b>'+esc(a)+'</b>　'+esc(b)+'</div>'}
function r(){let s=S(),b=h.querySelector(".v43body"),rt=typeof formatMechaRuntime==="function"?formatMechaRuntime(s.runtimeSeconds):"00:00:00";
if(p==="s")b.innerHTML='<div class="v43g">'+c("🕐 时间",s.storyTime||"时间未明确")+c("🔋 电量",(s.battery??100)+"%")+c("📶 网络",s.network||"OFFLINE")+c("🧠 CPU",(s.cpu??12)+"%")+c("🎛 GPU",(s.gpu??8)+"%")+c("💾 内存",(s.memory??18)+"%")+c("📍 位置",s.location||"位置未明确")+c("💧 冷却液",(s.coolant??100)+"%")+c("🌡 温度",(s.temperature??36.5)+"°C")+c("🛠 损伤",s.damage||"机体完整")+c("⏱ 运行时长",rt)+'</div>'+(s.alert?'<div class="v43box">⚠ '+esc(s.alert)+'</div>':'');
else if(p==="v")b.innerHTML='<div class="v43box">🔎 '+esc(s.scan||"当前无电子眼扫描记录")+'</div>'+c("定位",s.location||"位置未明确")+c("链路",s.network||"OFFLINE");
else if(p==="b")b.innerHTML='<div class="v43g">'+c("CPU",(s.cpu??12)+"%")+c("GPU",(s.gpu??8)+"%")+c("内存",(s.memory??18)+"%")+c("核心温度",(s.temperature??36.5)+"°C")+'</div><div class="v43box">⚙ '+esc(s.footer||"等待电子脑活动记录")+'</div>';
else b.innerHTML='<div class="v43box">时间：'+esc(s.storyTime||"时间未明确")+'</div>'+(s.scan?'<div class="v43box">扫描：'+esc(s.scan)+'</div>':'')+(s.alert?'<div class="v43box">警报：'+esc(s.alert)+'</div>':'');}
o.onclick=()=>{r();h.hidden=!h.hidden};h.querySelector(".v43h button").onclick=()=>h.hidden=true;h.querySelectorAll(".v43tabs button").forEach(x=>x.onclick=()=>{p=x.dataset.p;h.querySelectorAll(".v43tabs button").forEach(y=>y.classList.toggle("on",y===x));r()});setInterval(()=>{if(!h.hidden)r()},1000);
})();
