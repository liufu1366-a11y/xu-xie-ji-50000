...
function saveMechaDynamicState(s){
  ...
  catch(e){console.warn("[续写鸡·MECHA V40] 状态保存失败",e);}
  try{
    (window.top||window).dispatchEvent(new CustomEvent("xuxieji-mecha-state",{detail:state}));
  }catch(e){console.warn("[续写鸡·MECHA V40] 状态广播失败",e);}
}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V40] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V40")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V40] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V40",
    ...
    }catch(error){console.error("[续写鸡·MECHA V40] HUD失败",error);}
  },450);
}
...
/* ===== V40: EMBEDDED MECHA OS (original IIFE preserved) ===== */
﻿(async function () {
'use strict';
...
/* ===== V40 shared-state bridge ===== */
(function installV40MechaBridge(){
  const W = window.top || window;
  if (W.__XUXIEJI_V40_BRIDGE__) return;
  W.__XUXIEJI_V40_BRIDGE__ = true;

  W.addEventListener("xuxieji-mecha-state", function(ev){
    try {
      const s = ev && ev.detail;
      if (!s || typeof W.MechaOSStoryUpdate !== "function") return;
      W.MechaOSStoryUpdate({
        battery:s.battery, cpu:s.cpu, gpu:s.gpu, memory:s.memory,
        temperature:s.temperature, coolant:s.coolant,
        location:s.location, network:s.network,
        storyTime:s.storyTime,
        damage:s.damage ? [String(s.damage)] : [],
        vision:s.scan || "",
        activity:s.footer || "",
        log:s.scan ? ("扫描：" + s.scan) : "续写鸡状态同步"
      });
    } catch (e) {
      console.warn("[续写鸡 V40] MECHA OS 同步失败", e);
    }
  });
})();
