...
function saveMechaDynamicState(s){ try{localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(s||{}),updateTime:Date.now()}));}catch(e){console.warn("[续写鸡·MECHA V44] 状态保存失败",e);} }
...
function createMechaPreviewCard(spec) {
  ...
  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      ...
      <i>${escapeMechaInline(spec?.badge || "MECHA LIVE · V44")}</i>
    </span>
...
function scheduleMechaPreviewUi(storyText) {
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      ...
      for(let i=0;i<workPoints.length;i++){
        ...
        try{
          ...
          console.error("[续写鸡·MECHA V44] 动态扫描失败",err);
          spec={
            show:true,kind:"system",title:"S-R-1090",badge:"MECHA LIVE · V44",
    ...
    }catch(error){console.error("[续写鸡·MECHA V44] HUD失败",error);}
  },450);
}
...
/* ===== ORIGINAL MECHA_OS_V35_FIX3 (UNCHANGED UI) ===== */
(async function () {
'use strict';
...
/* ===== V44 BRIDGE: continuation state -> original MECHA OS FIX3 ===== */
(function(){
  const W=window.top||window;
  if(W.__XUXIEJI_V44_BRIDGE__)return;
  W.__XUXIEJI_V44_BRIDGE__=true;
  let last="";
  setInterval(function(){
    try{
      if(typeof loadMechaDynamicState!=="function"||typeof W.MechaOSStoryUpdate!=="function")return;
    ...
    }catch(e){console.warn("[续写鸡 V44] 原HUD同步失败",e);}
  },700);
})();
