...
function scheduleMechaPreviewUi(storyText) {
  const generator=getMechaPreviewGenerator();
  if(!generator){
    console.warn("[续写鸡·MECHA] 未发现 MechaOSGenerateChatUI");
    return;
  }
  if(mechaPreviewUiTimer){clearTimeout(mechaPreviewUiTimer);mechaPreviewUiTimer=null;}
  const token=++mechaPreviewUiToken;
  const input=String(storyText||"").trim();
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      const points=findMechaEventPoints(input);
      // 没找到事件时仍保留一个结尾状态节点，保证主 HUD 能结算本轮。
      const workPoints=points.length?points:[input.length];
      const rendered=[];
      let previous=0;

      for(let i=0;i<workPoints.length;i++){
        if(token!==mechaPreviewUiToken)return;
        const point=workPoints[i];
        // 给模型看“上一节点到当前节点”的剧情；最后一个节点额外吃到正文结尾。
        const isLast=i===workPoints.length-1;
        const end=isLast?input.length:point;
        const segment=input.slice(previous,end).trim();
        previous=end;
        if(segment.length<25)continue;

        const spec=await generator(null,segment);
        if(spec&&typeof spec==='object'){spec.__storyText=segment;spec.__storyChars=segment.length;}
        if(token!==mechaPreviewUiToken)return;
        if(spec&&spec.show===true){
          // HUD 放在该剧情事件结束位置；每个 spec 已经是该节点自己的冻结快照。
          rendered.push({index:end,spec});
        }
      }
      ...
      if(rendered.length){
        insertMultipleMechaUiIntoPreview(rendered,input);
      }else{
        previewSpan.html(escapeHtml(input));
        previewSpan.addClass("xuxieji-ai-continuation-mark");
      }
    }catch(error){
      console.error("[续写鸡·MECHA V34] 多事件 HUD 生成失败:",error);
      if(!editorDom||isEditorDestroyed||!hasLivePreviewSpan())return;
      const previewSpan=editorDom.find("#preview_content_span");
      previewSpan.html(escapeHtml(input));
      previewSpan.addClass("xuxieji-ai-continuation-mark");
