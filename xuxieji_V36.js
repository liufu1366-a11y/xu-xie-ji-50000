...
function getMechaDynamicStateStorageKey() {
  return getStrictStoryScopedKey(MECHA_DYNAMIC_STATE_KEY);
}
function getDefaultMechaDynamicState() {
  return { storyTime:"时间未明确", runtimeSeconds:0, battery:100, network:"OFFLINE",
    cpu:12, gpu:8, memory:18, location:"位置未明确", coolant:100, temperature:36.5,
    damage:"机体完整", scan:"", alert:"", footer:"状态系统已启动，等待剧情扫描", updateTime:Date.now() };
}
function loadMechaDynamicState() {
  try {
    const raw=localStorage.getItem(getMechaDynamicStateStorageKey());
    return raw ? {...getDefaultMechaDynamicState(),...JSON.parse(raw)} : getDefaultMechaDynamicState();
  } catch(e) { console.warn("[续写鸡·MECHA V36] 状态读取失败",e); return getDefaultMechaDynamicState(); }
}
function saveMechaDynamicState(state) {
  try { localStorage.setItem(getMechaDynamicStateStorageKey(),JSON.stringify({...getDefaultMechaDynamicState(),...(state||{}),updateTime:Date.now()})); }
  catch(e){ console.warn("[续写鸡·MECHA V36] 状态保存失败",e); }
}
function formatMechaRuntime(seconds) {
  const t=Math.max(0,Math.floor(Number(seconds)||0)), h=Math.floor(t/3600), m=Math.floor((t%3600)/60), s=t%60;
  return [h,m,s].map(v=>String(v).padStart(2,"0")).join(":");
}
function clampMechaNumber(value,min,max,fallback) {
  const n=Number(value); return Number.isFinite(n)?Math.max(min,Math.min(max,n)):fallback;
}
function parseMechaJson(raw) {
  const source=String(raw||"").trim().replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"");
  const a=source.indexOf("{"), b=source.lastIndexOf("}");
  if(a<0||b<=a) throw new Error("机娘状态分析未返回JSON");
  return JSON.parse(source.slice(a,b+1));
}
function normalizeMechaDynamicState(previous,incoming) {
  const p={...getDefaultMechaDynamicState(),...(previous||{})}, n=incoming&&typeof incoming==="object"?incoming:{};
  const elapsed=clampMechaNumber(n.elapsedSeconds,0,864000,0);
  return {
    storyTime:String(n.storyTime||p.storyTime||"时间未明确").slice(0,40),
    runtimeSeconds:Math.max(0,Number(p.runtimeSeconds)||0)+elapsed,
    battery:clampMechaNumber(n.battery,0,100,p.battery), network:String(n.network||p.network||"OFFLINE").slice(0,30),
    cpu:clampMechaNumber(n.cpu,0,100,p.cpu), gpu:clampMechaNumber(n.gpu,0,100,p.gpu),
    memory:clampMechaNumber(n.memory,0,100,p.memory), location:String(n.location||p.location||"位置未明确").slice(0,80),
    coolant:clampMechaNumber(n.coolant,0,100,p.coolant), temperature:clampMechaNumber(n.temperature,-100,300,p.temperature),
    damage:String(n.damage||p.damage||"机体完整").slice(0,80), scan:String(n.scan||"").slice(0,180),
    alert:String(n.alert||"").slice(0,180), footer:String(n.footer||"本轮剧情扫描完成，状态已结算").slice(0,180), updateTime:Date.now()
  };
}
function buildMechaDynamicSpec(s0) {
  const s={...getDefaultMechaDynamicState(),...(s0||{})}, rows=[];
  if(s.scan) rows.push({label:"扫描分析情况",value:s.scan});
  rows.push(
    {label:"时间",value:s.storyTime},{label:"电量",value:`${Number(s.battery).toFixed(1)}%`},
    {label:"网络",value:s.network},{label:"CPU",value:`${Math.round(Number(s.cpu)||0)}%`},
    {label:"GPU",value:`${Math.round(Number(s.gpu)||0)}%`},{label:"内存",value:`${Math.round(Number(s.memory)||0)}%`},
    {label:"位置",value:s.location},{label:"冷却液",value:`${Number(s.coolant).toFixed(1)}%`},
    {label:"温度",value:`${Number(s.temperature).toFixed(1)}°C`},{label:"损伤",value:s.damage},
    {label:"运行时长",value:formatMechaRuntime(s.runtimeSeconds)}
  );
  return {show:true,kind:"system",title:"S-R-1090",badge:"LIVE",rows,alert:s.alert||"",footer:s.footer||"本轮剧情扫描完成，状态已结算"};
}
async function analyzeMechaDynamicSegment(segment,previousState) {
  const preset=getActivePresetParams(), previous={...getDefaultMechaDynamicState(),...(previousState||{})};
  const systemPrompt=`你是小说中的机娘机体状态结算器。读取本轮剧情和上一状态，只返回可被JSON.parse解析的JSON。
规则：
1. 数值必须相对上一状态随剧情变化，禁止每轮重置成固定模板。
2. storyTime是故事内时间；出现几点、清晨、下午、夜晚、几小时后、次日等就推进，否则继承。不要使用现实手机时间。
3. elapsedSeconds是本片段经过的故事时间秒数；明确时间跨度按正文，短暂动作/对话可保守估计。
4. 电量、CPU、GPU、内存、冷却液、温度根据运动、战斗、扫描、运算、休息、充电、冷却、维修动态改变；无相关事件尽量保持。
5. location随剧情地点变化；damage随受击/维修变化；network只在联网/断网/信号变化时改变。
6. 出现扫描、侦测、识别、锁定、分析、检索、电子眼/电子脑分析或明显扫描目标时，scan必须写具体结果；否则为空。
7. alert仅在危险、异常、过热、低电量、损伤等情况出现。
只输出：
{"storyTime":"","elapsedSeconds":0,"battery":0,"network":"","cpu":0,"gpu":0,"memory":0,"location":"","coolant":0,"temperature":0,"damage":"","scan":"","alert":"","footer":""}`;
  ...
  return normalizeMechaDynamicState(previous,parseMechaJson(raw));
}
...
function createMechaPreviewCard(spec) {
  ...
  const rows = Array.isArray(spec?.rows) ? spec.rows.slice(0, 14) : [];
  const scanRows = rows.filter(r => /扫描|分析|目标/.test(String(r?.label||""))).slice(0,2);
  const stateRows = rows.filter(r => !/扫描|分析|目标/.test(String(r?.label||""))).slice(0,10);

  card.innerHTML = `
    <span class="xuxieji-mecha-v32-head">
      <b>${escapeMechaInline(spec?.title || "S-R-1090")}</b>
      <i>${escapeMechaInline(spec?.badge || "FROZEN")}</i>
    </span>
...
function findMechaEventPoints(text){
  const raw=String(text||"");
  const rx=/(扫描|侦测|识别|锁定|分析|检索|数据库|联网|导航|定位|警告|异常|故障|受损|损伤|攻击|战斗|射击|追踪|维修|修复|充电|补充能源|补充电力|冷却液|补充冷却液|加机油|过热|高负载|电子眼|电子脑)/g;
  const pts=[]; let m;
  while((m=rx.exec(raw))){
    if(m.index<80 || m.index>raw.length-80) continue;
    // 同一小段内多个关键词只算一个事件，避免疯狂刷 HUD。
    if(pts.length && m.index-pts[pts.length-1]<260) continue;
    let end=m.index;
    const tail=raw.slice(m.index,Math.min(raw.length,m.index+700));
    const sm=tail.match(/[。！？!?]["”’』」）)]?\s*/);
    if(sm) end=m.index+sm.index+sm[0].length;
    else end=Math.min(raw.length,m.index+220);
    pts.push(end);
  }
  // 长文即使事件词稀少，也按自然段补少量“状态节点”，但不是按固定字数硬塞。
  if(raw.length>2500 && pts.length<3){
    const para=[]; const pr=/\n\s*\n/g;
    while((m=pr.exec(raw))) if(m.index>raw.length*.15&&m.index<raw.length*.9) para.push(m.index+m[0].length);
    const targets=[.28,.55,.78];
    for(const t of targets){
      if(pts.length>=3)break;
      if(!para.length)break;
      const want=raw.length*t;
      const p=para.reduce((a,b)=>Math.abs(b-want)<Math.abs(a-want)?b:a,para[0]);
      if(!pts.some(x=>Math.abs(x-p)<300))pts.push(p);
    }
  }
  pts.sort((a,b)=>a-b);
  // 1万字允许更多事件，但设置上限防止 UI 淹没正文。
  const max=Math.max(3,Math.min(10,Math.ceil(raw.length/1200)));
...
function scheduleMechaPreviewUi(storyText) {
  const externalGenerator=getMechaPreviewGenerator();
  ...
  mechaPreviewUiTimer=setTimeout(async()=>{
    try{
      const points=findMechaEventPoints(input), workPoints=points.length?points:[input.length], rendered=[];
      let previous=0, workingState=loadMechaDynamicState();

      for(let i=0;i<workPoints.length;i++){
        if(token!==mechaPreviewUiToken)return;
        const point=workPoints[i], end=i===workPoints.length-1?input.length:point;
        const segment=input.slice(previous,end).trim(); previous=end;
        if(segment.length<25)continue;

        let spec=null;
        try{
          workingState=await analyzeMechaDynamicSegment(segment,workingState);
          spec=buildMechaDynamicSpec(workingState);
        }catch(dynamicError){
          console.warn("[续写鸡·MECHA V36] 动态扫描失败，尝试旧生成器",dynamicError);
          if(externalGenerator){ try{spec=await externalGenerator(null,segment);}catch(e){console.warn("[续写鸡·MECHA V36] 旧生成器也失败",e);} }
        }
      ...
      const previewSpan=editorDom.find("#preview_content_span");
      previewSpan.find(".xuxieji-mecha-ui-loading").remove();
      if(rendered.length)insertMultipleMechaUiIntoPreview(rendered,input);
      else{previewSpan.html(escapeHtml(input));previewSpan.addClass("xuxieji-ai-continuation-mark");}
    }catch(error){
      console.error("[续写鸡·MECHA V36] 动态HUD生成失败:",error);
      if(!editorDom||isEditorDestroyed||!hasLivePreviewSpan())return;
      const previewSpan=editorDom.find("#preview_content_span");
      previewSpan.find(".xuxieji-mecha-ui-loading").remove();
      previewSpan.html(escapeHtml(input)); previewSpan.addClass("xuxieji-ai-continuation-mark");
      toastr.warning("机娘HUD扫描分析失败，本轮正文仍可正常保存。","MECHA HUD");
    }
  },450);
}
